...
#############functions for simulation
    ...
    #SET KNOWN TRUTH (NEEDS TO RESET FOR EACH ROUND)
      createTruth <- function(B1, B2, B3, B4, B5 ,Bint1, Bint2) {
        ...
        #x1 and x6 values chosen from the calculated means of the quantiles
        ...
        Truth$y <- B1*Truth$x1 + B2*Truth$x2 + B3*Truth$x3 + B4*Truth$x4 + B5*Truth$x5 + Bint1*Truth$int1*Truth$x1*Truth$x2 + Bint2*Truth$int2*Truth$x3*Truth$x4
...
#############looping simulation
  ...
  for (n in n_list){
      ...
      results <- foreach (i=1:1000, .combine=rbind, .packages=c('rpart', 'partykit', 'tuneRanger', 'lme4', 'lsr', 'truncnorm', 'mlr', 'merTools', 'plyr')) %dopar% {
        ...
        b1 <- Sample.positive.coeff(1)
        b2 <- Sample.positive.coeff(1)
        b3 <- Sample.negative.coeff(1)
        b4 <- Sample.negative.coeff(1)
        b5 <- Sample.negative.coeff(1)
        b6 <- 0  
        bint1 <- Sample.negative.coeff(1)
        bint2 <- Sample.positive.coeff(1)
        
        D$y <- b1*D$x1 + b2*D$x2 + b3*D$x3 + b4*D$x4 + b5*D$x5 + bint1*D$int1*D$x1*D$x2 + bint2*D$int2*D$x3*D$x4*D$x5 + rnorm(n) 
        ...
        Truth<- createTruth(b1, b2, b3, b4, b5, bint1, bint2)
        #need to save the cluster id's in order of 1 to 192
        ...
        leafCART <- sum(ifelse(CARTpruned$frame == "<leaf>", 1, 0))
        ...
        #main effects (non-intersectional)
        ...
        MSEtable = c(i, NAcluster,
                     CART.predict.results, CTree.predict.results, RF.predict.results, 
                     BF.predict.results, OF.predict.results, MF.predict.results, 
      ...
      write.csv(results,paste("Cts_mixed", n, "csv", sep = "."))   

      
  }

  ################# Primary outcome presented by boxplots
  results2000<- read.csv ("T:/Cts_mixed.2000.csv")
  results5000<- read.csv ("T:/Cts_mixed.5000.csv")
  results50000<- read.csv ("T:/Cts_mixed.50000.csv")
  results200000<- read.csv ("T:/Cts_mixed.2e+05.csv")
  ...
  png(filename = "Mixed_inputs.png",width = 8, height = 4, units = 'in', res=300)
  p_final
  dev.off()
  ...
  a_list <- c( "T:/Cts_mixed.2000.csv",
               "T:/Cts_mixed.5000.csv",
               "T:/Cts_mixed.50000.csv",
               "T:/Cts_mixed.2e+05.csv")
  ...
  write.csv(output_ML_final,paste("Cts_mixed_ML_results", "csv", sep = "."))  
